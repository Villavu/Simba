...
uses
  LazFileUtils,
  simba.threading, simba.theme, simba.files, simba.settings, simba.scripttabsform;
...
begin
  ...
  begin
    ...
    if Assigned(LineInfo) and LineInfo.isFile then
      SimbaScriptTabsForm.Open(LineInfo.FileName);
  end;
end;
...
begin
  ...
  if Assigned(LineInfo) and SimbaScriptTabsForm.Open(LineInfo.FileName) then
    SimbaScriptTabsForm.CurrentTab.GotoLine(LineInfo.Line);
end;
