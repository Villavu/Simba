unit simba.script_import_tiomanager;
...
procedure TIOManager_SetImageTargetEx(const Params: PParamArray; const Result: Pointer); {$IFDEF Lape_CDECL}cdecl;{$ENDIF}
begin
  PTarget(Result)^ := PIOManager(Params^[0])^.SetImageTargetEx(POSWindow(Params^[1])^);
end;

procedure TIOManager_SetKeyMouseTargetEx(const Params: PParamArray; const Result: Pointer); {$IFDEF Lape_CDECL}cdecl;{$ENDIF}
begin
