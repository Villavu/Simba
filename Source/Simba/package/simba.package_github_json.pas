...
begin
  ...
  if (JSON <> nil) then
  begin
    if (JSON is TJSONArray) then Exit(TJSONArray(JSON));
...
begin
  ...
  try Result := Strings['zipball_url'];
  except
  end;
end;
...
begin
  ...
  if (Str <> '') then
  try Result := ScanDateTime('yyyy-mm-ddhh:nn:ss', Str);
  except
  end;
end;
