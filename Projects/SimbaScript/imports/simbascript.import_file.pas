...
begin
  PBoolean(Result)^ := DeleteDirectory(PString(Params^[1])^, PBoolean(Params^[2])^);
end;
...
begin
  UnZipOneFile(PString(Params^[1])^, PString(Params^[2])^, PString(Params^[3])^);
end;
...
begin
  ...
  begin
    ...
    addGlobalMethod('procedure UnZipOneFile(const ArchiveFileName, FileName, OutputPath: String);', @Lape_UnZipOneFile, Data);
