...
begin
  TSimbaScript(Params^[0]).Client.IOManager.SetTargetEx(PSysProc(Params^[1])^);
end;

procedure Lape_SetDesktopAsClient(const Params: PParamArray); {$IFDEF Lape_CDECL}cdecl;{$ENDIF}
begin
  TSimbaScript(Params^[0]).Client.IOManager.SetDesktop();
end;

procedure Lape_SetTargetArray(const Params: PParamArray; const Result: Pointer); {$IFDEF Lape_CDECL}cdecl;{$ENDIF}
begin
  PInt32(Result)^ := TSimbaScript(Params^[0]).Client.IOManager.SetTarget(PRGB32(PPtrUInt(Params^[1])^), Point(PInt32(Params^[2])^, PInt32(Params^[3])^));
end;
