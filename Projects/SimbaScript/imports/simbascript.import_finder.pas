...
begin
  with TSimbaScript(Params^[0]).Client do // bitmap                     out TPA                   formula                  xs                  ys                  xe                  ye                  min match              dynamic adjust
...
begin
  with TSimbaScript(Params^[0]).Client do // bitmap                   out X               out Y               formula                  xs                  ys,                 xe                  ye                  min match              dynamic adjust
...
begin
  with TSimbaScript(Params^[0]).Client do
    PIntegerMatrix(Result)^ := IOManager.ReturnMatrix(PInt32(Params^[1])^, PInt32(Params^[2])^, PInt32(Params^[3])^ - PInt32(Params^[1])^ + 1, PInt32(Params^[4])^ - PInt32(Params^[2])^ + 1);
end;
...
begin
  ...
  begin
    ...
    addGlobalMethod('function GetColorsMatrix(X1, Y1, X2, Y2: Int32): TIntegerMatrix;', @Lape_GetColorsMatrix, Data);
