...
begin
  PString(Result)^ := ScriptInstance.Client.MOCR.GetUpTextAt(7, 7, True);
end;

procedure Lape_rs_GetUpTextAt(const Params: PParamArray; const Result: Pointer); {$IFDEF Lape_CDECL}cdecl;{$ENDIF}
begin
  PString(Result)^ := ScriptInstance.Client.MOCR.GetUpTextAt(PInt32(Params^[0])^, PInt32(Params^[1])^, True);
end;

procedure Lape_rs_GetUpTextAtEx(const Params: PParamArray; const Result: Pointer); {$IFDEF Lape_CDECL}cdecl;{$ENDIF}
begin
  PString(Result)^ := ScriptInstance.Client.MOCR.GetUpTextAtEx(PInt32(Params^[0])^, PInt32(Params^[1])^, True, PString(Params^[2])^);
end;
