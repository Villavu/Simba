...
(* .simbapackage
  ...
  collapse=false
  // files not to install delimited with a comma.
...
type
  ...
  TPackageForm = class(TForm)
    ...
    procedure checkExtractChange(Sender: TObject);
...
uses
  LCLType, newsimbasettings, dateutils, MufasaTypes, httpd, Zipper, XMLRead, DOM, Math,
  {$IFDEF VER3_0} fphttpclient {$warn we can delete the package internet files now!}
  {$ELSE} package_fphttpclient
  {$ENDIF};
...
// Let's see how this works without multithreading. It's much more simple but will affect simba's GUI performace
// but i'm assuming people will keep the package form open while something is installing...
type
  TDownloader = class
  protected
    FData: TMemoryStream;
  ...
  public
    ...
    property Response: Int32 read FResponse;
...
begin
  if (Position > 0) and (Size > 0) then
    PackageForm.UpdateStatus('Downloading... ' + IntToStr(Round(Position / Size * 100)) + '%', True)
  else
...
function TDownloader.Get(URL: String): Boolean;
var
...
begin
  ...
  Client.OnDataReceived := @Downloading;

  try
    Client.HTTPMethod('GET', URL, FData, [HTTP_OK, HTTP_NOT_FOUND]);

    FResponse := Client.ResponseStatusCode;
  except
    on e: Exception do
    begin
      WriteLn('TDownloader.Download: ', e.ClassName, '::', e.Message);

      if (e is EHTTPClient) then
        FResponse := EHTTPClient(e).StatusCode;
    end;
  end;

  Client.Free();

  Result := (FResponse = HTTP_OK);

  PackageForm.UpdateStatus('', True);
end;

function TDownloader.Extract(Path: String; Collapse: Boolean;
  Blacklist: TStringList): Boolean;
var
  Zipper: TUnZipper;
  Archive: String;
begin
  ...
  Zipper.OutputPath := IncludeTrailingPathDelimiter(ExtractFilePath(Path));
    ...
    for i := 0 to high(Package.Releases) do
      ...
      begin
        ...
        begin
          ...
          if Downloader.Get(Package.Releases[i].ConfigURL) then
          begin
  ...
  begin
    ...
    try
      ...
      begin
        ...
        try
          if Downloader.Get(SelectedRelease.Download) then
          begin
            ...
            UpdateStatus('ERROR: Couldn''t access download URL: ' + IntToStr(Downloader.Response))
        finally
          Downloader.Free();
        end;
      end;
...
procedure TPackageForm.checkExtractChange(Sender: TObject);
begin
{
  checkExtractDirectory.OnChange := nil;
  checkExtractInDirectory.OnChange := nil;

  if (Sender = checkExtractDirectory) then
    checkExtractInDirectory.Checked := False;
  if (Sender = checkExtractInDirectory) then
    checkExtractDirectory.Checked := False;

  checkExtractDirectory.OnChange := @checkExtractChange;
  checkExtractInDirectory.OnChange := @checkExtractChange;
}
end;

procedure TPackageForm.installationChange(Sender: TObject);
begin
  GroupBox1.Enabled := btnCustomInstall.Checked;
end;
...
begin
  ...
  try
    if Downloader.Get(Format('https://api.github.com/repos/%s/%s/releases', [Package.Owner, Package.Name])) then
    begin
      ...
      UpdateStatus('ERROR: Couldn''t access releases API: ' + IntToStr(Downloader.Response))
  finally
    Downloader.Free();
  end;
