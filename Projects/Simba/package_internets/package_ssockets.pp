...
// This must be here, to prevent it from overriding the sockets definitions... :/
  SysUtils, Classes, ctypes, package_sockets;
...
function TSocketStream.GetLocalAddress: package_sockets.TSockAddr;
var
  len: LongInt;
begin
  len := SizeOf(package_sockets.TSockAddr);
  if fpGetSockName(Handle, @Result, @len) <> 0 then
    FillChar(Result, SizeOf(Result), 0);
end;

function TSocketStream.GetRemoteAddress: package_sockets.TSockAddr;
var
  len: LongInt;
begin
  len := SizeOf(package_sockets.TSockAddr);
...
{$else}
{$ifdef windows}
    for i := 0 to High(FDS.fd_array) do
      FDS.fd_array[i] := 0;
    FD_Zero(FDS);
    FD_Set(FSocket, FDS);
...
begin
  If Not FBound then
    Bind;
  If  package_sockets.FpListen(FSocket,FQueueSize)<>0 then
...
begin
  FHost:=aHost;
  FPort:=APort;
  S:=package_sockets.FpSocket(AF_INET,SOCK_STREAM,0);
  If S=-1 Then
...
begin
  ...
  if  package_sockets.fpBind(FSocket, @FAddr, Sizeof(FAddr))<>0 then
...
{$ENDIF UNIX}
   begin
   Result:=package_sockets.fpAccept(Socket,@Faddr,@L);
   R:=SocketError;
   end;
{$ifdef Unix}
  If (Result<0) then
...
begin
  ...
  addr.sin_addr.s_addr := htonl(a.s_addr);
  {$ifdef unix}
  Res:=ESysEINTR;
