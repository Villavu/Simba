...
begin
  ...
  begin
    ...
    addGlobalFunc('function TMOCR.getTextPointsIn(sx, sy, w, h: Integer; shadow: boolean; var _chars, _shadows: T2DPointArray): Boolean;', @TMOCR_getTextPointsIn);
    addGlobalFunc('function TMOCR.GetUpTextAtEx(atX, atY: integer; shadow: boolean; fontname: string): string;', @TMOCR_GetUpTextAtEx);
    addGlobalFunc('function TMOCR.GetUpTextAt(atX, atY: integer; shadow: boolean): string;', @TMOCR_GetUpTextAt);
    addGlobalFunc('procedure TMOCR.CreateDefaultFilter();', @TMOCR_CreateDefaultFilter);
    addGlobalFunc('procedure TMOCR.SetFilter(filter: TOCRFilterDataArray);', @TMOCR_SetFilter);
    addGlobalFunc('procedure TMOCR.ResetFilter();', @TMOCR_ResetFilter);
    addGlobalFunc('procedure TMOCR.FilterUpTextByColour(bmp: TMufasaBitmap);', @TMOCR_FilterUpTextByColour);
    addGlobalFunc('procedure TMOCR.FilterUpTextByCharacteristics(bmp: TMufasaBitmap);', @TMOCR_FilterUpTextByCharacteristics);
    addGlobalFunc('procedure TMOCR.FilterUpTextByColourMatches(bmp: TMufasaBitmap);', @TMOCR_FilterUpTextByColourMatches);
    addGlobalFunc('procedure TMOCR.FilterShadowBitmap(bmp: TMufasaBitmap);', @TMOCR_FilterShadowBitmap);
    addGlobalFunc('procedure TMOCR.FilterCharsBitmap(bmp: TMufasaBitmap);', @TMOCR_FilterCharsBitmap);
    addGlobalFunc('function TMOCR.GetTextAt(atX, atY, minvspacing, maxvspacing, hspacing, color, tol, len: integer; font: string): string;', @TMOCR_GetTextAt);
    addGlobalFunc('function TMOCR.GetTextAt(xs, ys, xe, ye, minvspacing, maxvspacing, hspacing, color, tol: integer; font: string): string; overload;', @TMOCR_GetTextAtEx);
    addGlobalFunc('function TMOCR.GetTextATPA(const ATPA: T2DPointArray; const maxvspacing: integer; font: string): string;', @TMOCR_GetTextATPA);
