...
begin
  ...
  begin
    ...
    addGlobalFunc('procedure TTarget.GetTargetDimensions(out w, h: integer);', @TTarget_GetTargetDimensions);
    addGlobalFunc('procedure TTarget.GetTargetPosition(out left, top: integer);', @TTarget_GetTargetPosition);
    addGlobalFunc('function TTarget.GetColor(x,y : integer): TColor;', @TTarget_GetColor);
    addGlobalFunc('function TTarget.ReturnData(xs, ys, width, height: Integer): TRetData;', @TTarget_ReturnData);
    addGlobalFunc('procedure TTarget.FreeReturnData();', @TTarget_FreeReturnData);
    addGlobalFunc('procedure TTarget.ActivateClient();', @TTarget_ActivateClient);
    addGlobalFunc('function TTarget.TargetValid(): boolean;', @TTarget_TargetValid);
    addGlobalFunc('function TTarget.MouseSetClientArea(x1, y1, x2, y2: integer): boolean;', @TTarget_MouseSetClientArea);
    addGlobalFunc('procedure TTarget.MouseResetClientArea();', @TTarget_MouseResetClientArea);
    addGlobalFunc('function TTarget.ImageSetClientArea(x1, y1, x2, y2: integer): boolean;', @TTarget_ImageSetClientArea);
    addGlobalFunc('procedure TTarget.ImageResetClientArea();', @TTarget_ImageResetClientArea);
    addGlobalFunc('function TTarget.GetError(): String;', @TTarget_GetError);
    addGlobalFunc('function TTarget.ReceivedError(): Boolean;', @TTarget_ReceivedError);
    addGlobalFunc('procedure TTarget.ResetError();', @TTarget_ResetError);
    addGlobalFunc('procedure TTarget.GetMousePosition(out x,y: integer);', @TTarget_GetMousePosition);
    addGlobalFunc('procedure TTarget.MoveMouse(x,y: integer);', @TTarget_MoveMouse);
    addGlobalFunc('procedure TTarget.ScrollMouse(x,y : integer; Lines : integer);', @TTarget_ScrollMouse);
    addGlobalFunc('procedure TTarget.HoldMouse(x,y: integer; button: TClickType);', @TTarget_HoldMouse);
    addGlobalFunc('procedure TTarget.ReleaseMouse(x,y: integer; button: TClickType);', @TTarget_ReleaseMouse);
    addGlobalFunc('function TTarget.IsMouseButtonHeld( button : TClickType): boolean;', @TTarget_IsMouseButtonHeld);
    addGlobalFunc('procedure TTarget.SendString(str: string; keywait, keymodwait: integer);', @TTarget_SendString);
    addGlobalFunc('procedure TTarget.HoldKey(key: integer);', @TTarget_HoldKey);
